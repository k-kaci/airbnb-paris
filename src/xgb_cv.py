...
def run(fold):
    ...
    model.fit(X_train, y_train, eval_set=[(X_valid, y_valid)], early_stopping_rounds=2, verbose=0)
    ...
    print("")
    joblib.dump(model, os.path.join(config.MODEL_OUTPUT, f"xgb_{fold}.bin"))
